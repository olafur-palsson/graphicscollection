...
const start = () => {
  ...
  const getTriangles = (nTriangles) => {
    const array = []
    for (let i = 0; i < nTriangles; i++) {
      const red = (150 + Math.random() * 50) / 250
      const green = (150 + Math.random() * 50) / 250
      const blue = (150 + Math.random() * 50) / 250
      const angle1 = Math.PI * 2 * i / nTriangles
      const angle2 = Math.PI * 2 * (i + 1) / nTriangles
      array.push(0, 0, 0, red, blue, green)
      array.push(Math.sin(angle1), 0, Math.cos(angle1), red, blue, green)
      array.push(Math.sin(angle2), 0, Math.cos(angle2), red, blue, green)
    }
  ...
  const slideEvent = () => {
    ...
    getTriangles(numTriangles)
  }

  slider.oninput = slideEvent

  setTimeout(() => { slideEvent(5); console.log('yolo') }, 500)

  // So we only call this function once
  const identityMatrix = new Float32Array(16)
  glMatrix.mat4.identity(identityMatrix)

  let rotateCircle = new Float32Array(16)
  let rotateMore = new Float32Array(16)
  let angle = 0
  ...
  const renderingLoop = () => {
    ...
    glMatrix.mat4.rotate(rotateCircle, identityMatrix, angle, [0, 1, 0])
    glMatrix.mat4.rotate(rotateMore, identityMatrix, angle * 0.5, [0.7, 1, 0])

    glMatrix.mat4.mul(mWorld, rotateMore, rotateCircle)
